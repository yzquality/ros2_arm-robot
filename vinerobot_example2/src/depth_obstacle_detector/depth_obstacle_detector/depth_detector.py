    ...
    def run_robot_arm(self):
        ...
        for i, target_positions in enumerate(target_positions_list):
            ...
            self.get_logger().info(
                f"🛠 arm robot move to Point {i+1}: {target_positions}, time is {duration:.2f}s"
            )
        ...
        if future.result().accepted:
            self.get_logger().info("✅ Arm robot run successfully!")
        else:
            self.get_logger().error("❌ Arm robot error!")
